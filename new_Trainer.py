...
class Trainer(object):
    def __init__(self, args, model=None,train_loader=None, val_loader=None,weighted_train_loader=None,per_class_num=[],log=None):
        ...
        self.train_scheduler = self.configure_scheduler()
        self.log = log
        self.beta = args.beta
        self.update_weight()
    ...
    def configure_optimizer(self):
        if self.args.optimizer_type == 'SGD':
            optimizer = torch.optim.SGD(
                ...
                lr=self.lr,
                momentum=self.args.momentum,  # 使用args中的momentum
            ...
            optimizer = torch.optim.RMSprop(
                ...
                lr=self.lr,
                alpha=0.9,
                momentum=self.args.momentum,  # RMSprop也可能使用momentum
                weight_decay=self.args.weight_decay
            )
        # ...可以添加更多的优化器选项...
        else:
            raise ValueError(f"Unsupported optimizer type: {self.args.optimizer_type}")
        return optimizer


    def configure_scheduler(self):
        if self.args.scheduler_type == 'CosineAnnealingLR':
        ...
        # ...可以添加更多的scheduler选项...
            ...
            raise ValueError(f"Unsupported scheduler type: {self.args.scheduler_type}")
        return scheduler
    ...
    def train_one_epoch(self):
        ...
        for i, (inputs, targets, _) in enumerate(train_loader):
            ...
            loss = self.criterion(output_cb, reweighted_targets if ((self.args.aug == "mixup" and self.args.mixup >= 0) or self.args.aug == "cutmix") else targets)
    ...
    def train_base(self):
        ...
        if self.args.loss == 'ce':
            ...
            self.criterion = nn.CrossEntropyLoss(label_smoothing=self.args.eps)
            ...
            if self.args.debug > 0:
                if (epoch + 1) % self.args.debug == 0:
                    ...
                    wandb.log({'nc/loss': nc_dict['loss'],
                               ...
                               'w_norm': nc_dict['w_norm'],
                               'h_norm': nc_dict['h_norm'],
                               },
                              step=epoch+1)
